...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-uploads-app runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-uploads-app.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_uploads_app")
...
def lint(c):
    ...
    """
    c.run("flake8 django-uploads-app tests")
...
def release(c, bumpsize=''):
    ...
    import django_uploads_app
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_uploads_app.__version__))
    c.run("git push --tags")
